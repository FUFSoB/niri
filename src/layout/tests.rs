...
use niri_config::{
    CenterFocusedColumn, FloatOrInt, FloatingPosition, OutputName, RelativeTo, Struts,
...
#[test]
fn focus_floating_switches_from_sticky_to_workspace_floating_when_available() {
    let ops = [
        Op::AddOutput(1),
        Op::AddWindow {
            params: TestWindowParams::new(1),
    ...
    let workspace = &monitors[*active_monitor_idx].workspaces
...
#[test]
fn switch_focus_floating_tiling_switches_from_sticky_to_workspace_floating() {
    let ops = [
        Op::AddOutput(1),
        Op::AddWindow {
            params: TestWindowParams::new(1),
...
#[test]
fn interactive_move_floating_with_rule_also_snaps_to_center_with_ctrl() {
    let ops = [
        Op::AddOutput(1),
        Op::AddWindow {
            params: TestWindowParams {
    ...
    let mut layout = check_ops(ops);
    let output = layout.outputs().next().unwrap().clone();
    let pointer = Point::from((612., 282.));
    ...
    let hint = move_
        ...
        .as_ref()
        .expect("floating hint should be active");
    assert_eq!(hint.rects[0].loc, Point::from((100., 50.)));
    assert_eq!(hint.snapped_tile_render_loc, Point::from((590., 260.)));
    ...
    let (_, pos, _) = ws
        ...
        .find(|(tile, _, _)| tile.window().id() == &1)
        .expect("window should remain on the active workspace");
    assert_eq!(pos, Point::from((590., 260.)));
}

#[test]
fn interactive_move_floating_with_top_right_rule_snaps_to_center_with_ctrl() {
    let ops = [
        Op::AddOutput(1),
        Op::AddWindow {
            params: TestWindowParams {
                ...
                rules: Some(ResolvedWindowRules {
                    default_floating_position: Some(FloatingPosition {
                        x: FloatOrInt(100.),
                        y: FloatOrInt(50.),
                        relative_to: RelativeTo::TopRight,
                    }),
                    ..ResolvedWindowRules::default()
                }),
                ..TestWindowParams::new(1)
    ...
    let hint = move_
        ...
        .as_ref()
        .expect("floating hint should be active");
    assert_eq!(hint.rects[0].loc, Point::from((1080., 50.)));
    assert_eq!(hint.snapped_tile_render_loc, Point::from((590., 260.)));
...
#[test]
fn interactive_move_floating_with_top_right_rule_snaps_to_center_with_real_grab_point() {
    let ops = [
        Op::AddOutput(1),
        Op::AddWindow {
            params: TestWindowParams {
    ...
    let grab_offset = Point::from((75., 20.));
    let start = pos + grab_offset;
    let pointer = Point::from((687., 302.));

    layout.interactive_move_begin(1, &output, start);
    layout.interactive_move_update(&1, pointer - start, output.clone(), pointer, true);
...
#[test]
fn interactive_move_floating_with_top_right_rule_snaps_when_ctrl_is_pressed_mid_drag() {
    let ops = [
        Op::AddOutput(1),
        Op::AddWindow {
            params: TestWindowParams {
    ...
    layout.interactive_move_begin(1, &output, start);
    layout.interactive_move_update(&1, pointer - start, output.clone(), pointer, false);
    ...
    let hint = move_
        ...
        .as_ref()
        .expect("floating hint should be active");
    assert_eq!(hint.snapped_tile_render_loc, Point::from((612., 282.)));

    assert!(layout.interactive_move_set_snap_enabled(true));
...
#[test]
fn interactive_move_sticky_with_top_right_rule_snaps_to_center_with_ctrl() {
    let ops = [
        Op::AddOutput(1),
        Op::AddWindow {
            params: TestWindowParams {
...
#[test]
fn interactive_move_sticky_with_top_right_rule_snaps_when_ctrl_is_pressed_mid_drag() {
    let ops = [
        Op::AddOutput(1),
        Op::AddWindow {
            params: TestWindowParams {
    ...
    let hint = move_
        .floating_hint
        .as_ref()
        .expect("sticky move should still show a hint");
    assert_eq!(hint.snapped_tile_render_loc, Point::from((590., 260.)));
}
